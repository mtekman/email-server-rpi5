...
/* Modified from  https://github.com/hoelzro/linux-fake-battery-module */
...
static char *fake_ac_supplies[] = {"BAT0"};
...
static enum power_supply_property fake_battery_properties[] = {
    ...
    POWER_SUPPLY_PROP_SERIAL_NUMBER,     POWER_SUPPLY_PROP_TEMP,
...
control_device_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
    ...
    if(count < message_len) {return -EINVAL;}
    if(*ppos != 0) {return 0;}
...
control_device_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    ...
    power_supply_changed(supplies[0]);  // BAT0
    power_supply_changed(supplies[1]);  // AC0
